  ...
  if (i < 3) {
    return (
      <span className="inline-flex items-center gap-1.5">
        <span
          className={`inline-grid place-items-center h-8 w-8 rounded-lg bg-gradient-to-b ${tiers[i]} text-black font-black text-sm shadow-[0_0_14px_-2px_rgba(212,175,55,0.6)] border border-white/30`}
        >
          {i + 1}
        </span>
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "app_settings" }, () =>
        supabase
          ...
          .eq("id", 1)
          .maybeSingle()
          .then(({ data }) => setHeaderUrl((data as any)?.leaderboard_header_url || leaderboardHeaderAsset.url)),
      )
      .subscribe();
    return () => {
        ...
        {headerUrl ? (
          ...
          <div className="relative mb-6 rounded-2xl overflow-hidden bg-black/20 backdrop-blur-[2px] border-2 border-amber-400/60 shadow-[0_0_40px_-10px_rgba(212,175,55,0.55)]">
            ...
            <div className="relative flex items-center gap-3 px-5 py-5">
              ...
              <h1 className="text-3xl md:text-4xl font-black tracking-tight gradient-gold-text drop-shadow-[0_2px_8px_rgba(212,175,55,0.3)]">
                LEADERBOARD
              </h1>
            </div>
          </div>
          ...
          <tbody>
            ...
            {rows.map((r, i) => (
              <tr key={r.name} className="border-b border-amber-400/10 hover:bg-amber-400/10 transition-colors">
                ...
                <Td>
                  {firstPick ? (
                    <span className="font-bold text-primary/90">{firstPick(r)}</span>
                  ) : (
                    <span className="font-bold">{pick(r)}</span>
                  )}
                </Td>
                <Td>
                  <span className={firstPick ? "font-bold" : "text-muted-foreground"}>
                    {firstPick ? pick(r) : r.top_player || "—"}
                  </span>
                </Td>
                <Td right>
                  <Pill tone="amber">{r.TS}</Pill>
                </Td>
                <Td right>
                  <span className="text-emerald-400 font-bold">{r.W}</span>
                </Td>
                <Td right>
                  <span className="text-destructive font-bold">{r.L}</span>
                </Td>
                <Td right>
                  <span className="text-amber-400 font-bold">{r.D}</span>
                </Td>
                <Td right>
                  <span className="text-muted-foreground">{r.P}</span>
                </Td>
                <Td right>
                  <Pill tone="emerald">{r.PTS}</Pill>
                </Td>
              </tr>
            ))}
          </tbody>
...
function Pill({ children, tone }: { children: React.ReactNode; tone: "amber" | "emerald" }) {
  ...
  return (
    <span
      className={`inline-grid place-items-center min-w-[40px] rounded-md border px-2 py-1 font-black tabular-nums ${cls}`}
    >
      {children}
    </span>
  );
